...
class Game(Tk):
    def __init__(self):
        ...
        self.train = Train(self.playSpace)
        ...
        self.frame = Frame(self.menuSpace, width= 400, height= 100, bg="pink")
    ...
    def resize_image(self):
            ...
            x = int(largeur/(NB_WAGONS+1)) * (NB_WAGONS+1)
            y = int(hauteur/4) * 4
            ...
            print(f'Largeur {largeur}({x})\nHauteur {hauteur} ({y})')
            #self.train.config(width = largeur/(NB_WAGONS+1))
            #self.train.config(height = hauteur/100)
            wid = int(largeur/(NB_WAGONS+1))
            hei = int(hauteur / 4)
            print(f'wid {wid} hei {hei}')
            
            locoImg = Game.createImgWithTransBg(wid, hei, "png/locomotive val.png")
            wagonImg = Game.createImgWithTransBg(wid, hei, "png/wagon val.png")
            queueImg = Game.createImgWithTransBg(wid, hei, "png/queue val.png")
 
        
            self.train.wagons[0].config(image=locoImg)
            self.train.wagons[0].image = locoImg

            for i in range(1, NB_WAGONS):
                self.train.wagons[i].config(image = wagonImg)
                self.train.wagons[i].image = wagonImg
            
            self.train.wagons[NB_WAGONS].config(image = queueImg)
            self.train.wagons[NB_WAGONS].image = queueImg
...
class Train(Canvas):
    ...
    def __init__(self, fenetre:Tk):
        # super().__init__(fenetre, width=width, height=height, bg='green', border=0)
        super().__init__(fenetre, bg='green', border=0)
...
class Wagon(Label):
    def __init__(self, Train:Canvas, x , y, tetewagonqueue):
        ...
        if tetewagonqueue == 0:
            ...
            self.img = Game.createImg(x, y, "png/locomotive val.png")
        elif tetewagonqueue == 1:
            # self.img = self.drawImage("png/wagon val.png")
            self.img = Game.createImg(x, y, "png/wagon val.png")
        elif tetewagonqueue == 2:
            # self.img = self.drawImage("png/queue val.png")
            self.img = Game.createImg(x, y, "png/queue val.png")
